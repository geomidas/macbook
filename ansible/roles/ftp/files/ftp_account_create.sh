...
# <--- Info ------------------------------------------------------------------>
#
# Description:  Creates local user account and ftp dir for client data
...
if [ "$#" -lt 1 ]; then
    ...
    echo 'Usage:'
    echo '/root/create-sftp-account.bash username'
    echo 'or'
    echo '/root/create-sftp-account.bash username password'
    exit
fi
...
useradd -m -d "/ftp/$username" -G sftp -s /sbin/nologin -p "$( echo $password | openssl passwd -1 -stdin )" $username
# Check for errors
exitcode=$( echo $? )
if [ $exitcode -eq '0' ] ; then
    # Make the user dir private
    echo -ne "Securing /ftp/$username \r"
    chown "${user}:ftpadmin" "/ftp/$username"
...
# Set permissions for ftp accounts
chmod 750 /ftp/*